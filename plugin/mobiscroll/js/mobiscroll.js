...
(function ($, window, document, undefined) {
    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function scroll(t, index, val, time, active) {
            ...
            if (has3d) {
                style[pr + 'Transform'] = 'translate3d(0,' + px/50 + 'rem,0)';
            } else {
                style.top = px/50 + 'rem';
            }
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += '<' + (hasFlex ? 'div' : 'td') + ' class="dwfl"' + ' style="' +
                                    (s.fixedWidth ? ('width:' + (s.fixedWidth[l] || s.fixedWidth)/50 + 'rem;') :
                                    (s.minWidth ? ('min-width:' + (s.minWidth[l] || s.minWidth)/50 + 'rem;') : 'min-width:' + s.width/50 + 'rem;') +
                                    (s.maxWidth ? ('max-width:' + (s.maxWidth[l] || s.maxWidth)/50 + 'rem;') : '')) + '">' +
                                '<div class="dwwl dwwl' + l + (w.multiple ? ' dwwms' : '') + '">' +
                                (s.mode != 'scroller' ?
                                    '<div class="dwb-e dwwb dwwbp ' + (s.btnPlusClass || '') + '" style="height:' + itemHeight/50 + 'rem;line-height:' + itemHeight/50 + 'rem;"><span>+</span></div>' + // + button
                                    '<div class="dwb-e dwwb dwwbm ' + (s.btnMinusClass || '') + '" style="height:' + itemHeight/50 + 'rem;line-height:' + itemHeight/50 + 'rem;"><span>&ndash;</span></div>' : '') + // - button
